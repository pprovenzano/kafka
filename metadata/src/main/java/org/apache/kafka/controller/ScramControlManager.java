...
public class ScramControlManager {
    ...
    class ScramControlIterator implements Iterator<List<ApiMessageAndVersion>> {
        private final int maxRecordsPerBatch;
        private final Iterator<Entry<ScramCredentialKey, ScramCredentialValue>> iterator;

        ScramControlIterator(int maxRecordsPerBatch, long epoch) {
            this.maxRecordsPerBatch = maxRecordsPerBatch;
            this.iterator = credentials.entrySet(epoch).iterator();
        }
        ...
        public List<ApiMessageAndVersion> next() {
            ...
            List<ApiMessageAndVersion> records = new ArrayList<>();
            int i = 0;
            while (iterator.hasNext() && i < maxRecordsPerBatch) {
                Entry<ScramCredentialKey, ScramCredentialValue> entry = iterator.next();
                records.add(toRecord(entry.getKey(), entry.getValue()));
                i++;
            }
            return records;
        }
    ...
    ScramControlIterator iterator(long epoch) {
        return new ScramControlIterator(10, epoch);
    }
}
